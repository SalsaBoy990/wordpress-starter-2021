...
namespace Guland\ReactWPBlocks\WooCommerce;
...
trait WooCommerce
{
   ...
   * @return string|null
   */
  public function renderLatestProductsBlock(array $attributes): string
  {
    $args = array(
    ...
    if ($products && count($products) >= 1) {
      ...
      return '<div class="wp-block-reactwp-blocks-latest-products row"><p>' . __('No Products Found', GULAND_REACTWP_BLOCKS_TEXT_DOMAIN) . '</p></div>';
    }
  ...
  public function getProductList(array $products, string $title = ''): string
  {
    ...
    $html .= '<div class="wp-block-reactwp-blocks-latest-products row row-cols-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-3 row-cols-xxl-4 g-4">';
